...
{
    [Header("自分自身の情報")]
    public int myID; // インスペクターで 0～5 を設定
    ...
    private int now_damage;

    void Start()
{
    AttackButton.gameObject.SetActive(false);
    is_selected = false;
    battleDataforOnline.character_isSelected[1] = false;
    ...
    void SyncUIPositionFromGrid()
{
    ...
    void Update()
    {
        battleDataforOnline.charactersBattleDatas[1].now_character_position.x = on_grid_number_x;
        battleDataforOnline.charactersBattleDatas[1].now_character_position.y = on_grid_number_y;
    ...
    public void OnMySelected() // ボタンクリックの代わりにこれを呼ぶ
    {
        ...
        is_selected = true;
        battleDataforOnline.character_isSelected[1] = true;
    ...
    void TryMove(int moveX, int moveY, Vector2 posDelta)
{
    ...
    if (battleDataforLocal.now_my_cost - 10 < 0) 
    {
        Debug.Log("コスト不足！");
        return;
    }
    ...
    if (gridDataforOnline.grid_state_y[nextY].grid_state_x[nextX] >= 0)
    {
        ...
        battleDataforLocal.now_my_cost -= 10;
